...
def run():
    ...
    # Build model
    model = Sequential()
    model.add(Dense(units=500, kernel_initializer='uniform', activation='relu', input_dim=1, input_shape=(x_train.shape[1], 1)))
    model.add(Dropout(.2))
    ...
    for i in range(days):
        ...
        predictions = np.concatenate([predictions, curr_prediction[0][0]])
